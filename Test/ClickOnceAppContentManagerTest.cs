...
public class ClickOnceAppContentManagerTest
{
    ...
    {
        using var iconStream = File.OpenRead(this.PathOf("z (16, 32, 48, 64, 128, 256px).ico"));
    ...
    public void ConvertIconToPng_Test(int pxSize)
    {
        ...
        var expectedPngBytes = File.ReadAllBytes(this.PathOf(expectedImagePath));
    ...
    {
        using var iconStream = File.OpenRead(this.PathOf("x (16, 32, 64px).ico"));
        using var pngStream = new MemoryStream();
        ClickOnceAppContentManager.ConvertIconToPng(iconStream, pngStream, pxSize);
        var actualPngBytes = pngStream.ToArray();
